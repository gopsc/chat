...
# 生成system.txt的绝对路径
path = os.path.join(basedir, 'system.txt')
...
with open("../cert/deepseek.key", "r") as f:
...
def chat_with_gpt(user: str):
    # 添加
    MySettings.append({"role": "user", "content": user})
    # 调用GPT
    ...
    MySettings.append({"role": "assistant", "content": content})
    if len(MySettings) > 10:
        del MySettings[1]
        del MySettings[1]
                ...
                for notified_socket in read_sockets:
                    if notified_socket == servsock:
                        ...
                        try:
                            ...
                            # 聊天发起
                            msg = allmsg[allmsg.find(": ")+2:]
                            if data and msg.startswith("<chatgpt>"):
                                msg = msg[msg.find('>') + 1:]
                                chatgpt = chat_with_gpt(msg)
